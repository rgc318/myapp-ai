...
class TestPromptRegistry(TestCase):
	def test_registry_covers_every_supported_scenario(self):
		...
		self.assertEqual(PROMPT_REGISTRY["intent_parse"].version, "erp-intent-v8")
		...
		self.assertEqual(PROMPT_REGISTRY["sales_order_draft"].version, "sales-order-draft-v5")
		self.assertIn("未提及的订单行必须保留", PROMPT_REGISTRY["sales_order_draft"].text)
		...
		self.assertIn("header_patch.clear_fields", PROMPT_REGISTRY["sales_order_draft"].text)
		self.assertIn("每一行都必须使用 add", PROMPT_REGISTRY["sales_order_draft"].text)
		self.assertEqual(PROMPT_REGISTRY["purchase_order_draft"].version, "purchase-order-draft-v5")
		self.assertIn("未提及的订单行必须保留", PROMPT_REGISTRY["purchase_order_draft"].text)
	...
	def test_endpoint_can_force_its_structured_scenario(self):
		...
		self.assertEqual(resolved.prompt_version, "inventory-adjustment-draft-v3")
