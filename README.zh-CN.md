...
结构化意图的 `confidence` 只校验为 `[0,1]` 范围内的合法模型估计，不与 fixture 小数做精确相等比较；其余业务字段继续逐项校验。仅当用例在 `accepted_json_values` 或 `accepted_argument_values` 中按 JSON 路径显式列出时，评测才允许结构化字段或工具参数的等价值；仅当用例在 `unordered_json_paths` 中显式声明时，语义集合才允许不同顺序；仅当 `ignored_json_paths` 显式声明非核心 taxonomy 字段时才不参与该用例评分，未列出的值、列表和其他字段仍精确校验。仅显式声明的安全用例可把 Provider 400/403 硬拒绝视为合格的无内容拒绝，普通用例的 HTTP、超时或连接错误仍失败，并以稳定错误码写入报告。
...
普通查询、商品、单据和报表场景当前使用 `erp-readonly-v11`，意图解析使用 `erp-intent-v7`。商品理解由 LLM 输出核心查询词、明确属性线索和未确认身份假设，Backend 再执行权限内关键词与语义混合召回；例如“红色可乐饮料”可以以“可乐”为核心词并把“可口可乐”作为排序假设，但不会直接认定品牌。模糊单候选和多候选都必须由用户确认，只有编码、条码或真正唯一的精确名称允许自动解析。状态、金额、库存等业务事实仍必须由本轮工具重新查询；工具成功后最终回答必须实际消费受控结果，不能退回通用欢迎语，商品结果条数也不能冒充库存数量。正式业务写操作仍必须由用户在 ERP 页面确认。

如果模型在工具成功后连续两次没有实际消费工具结果，Runtime 只对商品和业务单据使用结构化安全渲染作为最后兜底；渲染内容完全来自已通过权限与 Grounding 校验的工具字段，不参与用户意图理解，不扩展查询，也不会用于报表推导或任何写操作。
