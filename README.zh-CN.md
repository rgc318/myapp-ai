...
商品建档草稿使用 `product-setup-draft-v3`，区分标准售价、批发价、零售价和成本价（默认采购价）；旧 `valuation_rate` 仅用于兼容已有响应。所有价格必须来自用户原文；`currency` 只接受用户明确给出的 ISO 4217 代码或完整币种名称，金额后缀“元”或 `¥/￥` 不单独构成币种。正式价格表和首次入库成本仍由 Frappe 在用户确认后写入。
