...
销售、采购和商品 Prompt 版本分别为 `sales-order-draft-v3`、`purchase-order-draft-v3`、`product-setup-draft-v5`。每条 `ChatMessage.attachments` 最多 4 个，每项包含 `attachment_id / filename / mime_type / sha256 / width / height / data_base64`；Orchestrator 校验 base64、8MB 规范化后上限和 SHA-256，并把图片组装到其所属 user 消息的 OpenAI `image_url` 内容块。旧调用仍可使用顶层 `ChatRequest.attachments`，这些附件只兼容附加到最后一条 user 消息；同一 Attachment ID 已出现在消息级字段时不会重复发送。图片 base64 不进入文本 Token 估算或 Langfuse input；运行预算覆盖所有消息级和兼容顶层图片，按规范化后的尺寸估算每张图片为 `85 + 170 × ceil(width/512) × ceil(height/512)` 个输入 Token，旧调用缺少尺寸时按 1024×1024 保守估算。
