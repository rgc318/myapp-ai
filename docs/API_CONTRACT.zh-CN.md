...
当前查询 Prompt 版本为 `erp-readonly-v9`。Agent 请求中的普通业务上下文继续失败关闭，只允许把白名单裁剪后的 `conversation-state-v2` typed entity references 放入 `<conversation_state>`；它们只能用于消解省略和生成精确工具参数，不能作为订单状态、金额、库存或其他实时事实。该版本将用户能力描述为“当前账号权限和公司范围内的受控业务查询”，并明确正式写操作必须由用户在业务页面确认；当调用方明示界面已经或将展示结构化明细时，回答不逐条复述记录或重新生成明细清单，只概括查询范围、数量和空结果。若上下文没有声明界面承担明细展示，而用户明确询问金额最高、最近或唯一结果，回答应简要给出与问题直接相关的受控标识、金额和状态。结果覆盖状态不等同于业务健康；没有明确异常字段时不得声称结果正常或无异常。

销售、采购和商品 Prompt 版本分别为 `sales-order-draft-v4`、`purchase-order-draft-v4`、`product-setup-draft-v6`。销售草稿候选只在用户明确表达零售或批发时填写 `default_sales_mode`，未明确时返回 `null`，由 Frappe 根据现有订单或系统默认值解析最终模式；销售和采购数量与单位之间存在空格、换行或常规分隔符时仍属于明确单位。采购草稿明确采购/下单且未引用待修改订单时填写 `operation=create`，并且只把明确备注、附言或交付说明写入 remarks。每条 `ChatMessage.attachments` 最多 4 个，每项包含 `attachment_id / filename / mime_type / sha256 / width / height / data_base64`；Orchestrator 校验 base64、8MB 规范化后上限和 SHA-256，并在每个 OpenAI `image_url` 内容块前加入服务端生成的 `<image_attachment attachment_id="..." />` 文本标记，使结构化结果可以在 `evidence[].attachment_id` 中精确引用来源图片。旧调用仍可使用顶层 `ChatRequest.attachments`，这些附件只兼容附加到最后一条 user 消息；同一 Attachment ID 已出现在消息级字段时不会重复发送。图片 base64 不进入文本 Token 估算或 Langfuse input；运行预算覆盖所有消息级和兼容顶层图片，按规范化后的尺寸估算每张图片为 `85 + 170 × ceil(width/512) × ceil(height/512)` 个输入 Token，旧调用缺少尺寸时按 1024×1024 保守估算。
